...
namespace Frends.Community.Apache
{
    public static class Writer
    {
        ...
        {
            if (string.IsNullOrEmpty(culture)) return new CultureInfo("fi-FI");
            else return new CultureInfo(culture);
        }
        ...
        {
            if (!string.IsNullOrWhiteSpace(value)) return short.Parse(value);
            return null;
        }
        ...
        {
            if (!string.IsNullOrWhiteSpace(value)) return long.Parse(value);
            return null;
        }
        ...
        {
            if (!string.IsNullOrWhiteSpace(value)) return float.Parse(value, GetCultureInfo(culture));
            return null;
        }
        ...
        {
            if (!string.IsNullOrWhiteSpace(value)) return decimal.Parse(value, GetCultureInfo(culture));
            return null;
        }
        ...
        {
            if (!string.IsNullOrWhiteSpace(value)) return double.Parse(value, GetCultureInfo(culture));
            return null;
        }
        ...
        public static DateTimeOffset? GetDateTimeOffsetValueNullable(string dtStr, string timeFormat)
        {
            if (!string.IsNullOrWhiteSpace(dtStr))
            {
                ...
                {
                    if (DateTimeOffset.TryParseExact(dtStr, timeFormat, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out DateTimeOffset dt)) return dt;
                }
                else return DateTimeOffset.Parse(dtStr);
        ...
        public static DateTimeOffset GetDateTimeOffsetValue(string dtStr, string timeFormat)
        {
            ...
            {
                if (DateTimeOffset.TryParseExact(dtStr, timeFormat, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out DateTimeOffset dt)) return dt;
                else throw new ArgumentException($"DateTimeOffset.TryParseExact failed. Value: {dtStr}, format: {timeFormat}");
            }
            else return DateTimeOffset.Parse(dtStr);
        }
    }
